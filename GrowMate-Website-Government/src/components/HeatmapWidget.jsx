...
const getIntensityColor = (intensity) => {
  if (intensity >= 80) return "bg-[#DAA520]"; // Highlight color for highest intensity
  if (intensity >= 60) return "bg-[#004080]"; // Accent color for high intensity
  if (intensity >= 40) return "bg-[#2E7D32]"; // Primary color for medium intensity
  if (intensity >= 20) return "bg-[#66BB6A]"; // Secondary color for low intensity
  return "bg-[#4E4E4E]"; // Text secondary for very low intensity
};
    ...
    <Card className="animate-in fade-in-0 slide-in-from-left-4 duration-700 delay-200 bg-[#fff3d4]/45">
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>Farmer Registration Heatmap</span>
        </CardTitle>
        <CardDescription>
          District-wise farmer registration density
        </CardDescription>
      </CardHeader>
      <CardContent>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {getIntensityLabel(district.intensity)} Density
                  </p>
                </div>
              </div>
        ...
        <div className="mt-6 pt-4 border-t border-border">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              { color: "bg-[#DAA520]", label: "Very High (80%+)" },
              { color: "bg-[#004080]", label: "High (60-79%)" },
              { color: "bg-[#2E7D32]", label: "Medium (40-59%)" },
              { color: "bg-[#66BB6A]", label: "Low (20-39%)" },
              { color: "bg-[#4E4E4E]", label: "Very Low (<20%)" },
            ].map((item) => (
